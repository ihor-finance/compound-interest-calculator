# Google Play — матеріали для релізу

Усе, що потрібно вставити в Play Console.

**Тексти всіма 43 мовами** лежать у [store-assets/listings/](store-assets/listings/) —
по файлу на мову, з назвою, коротким і повним описом та підписами до карток.
`npm run test:listings` перевіряє, що жоден текст не виходить за ліміти Play.
...
| Мови в магазині | 43 — див. `store-assets/listings/` |
...
|---|---|---|
| Іконка застосунку | 512 × 512 PNG | ✅ `store-assets/icon-512.png` |
| Feature graphic | 1024 × 500 PNG | ✅ `store-assets/feature-graphic.png` |
| Скриншоти телефона | 1080 × 1920, 7 шт. × 43 мови | ✅ генеруються |

### Як згенерувати скриншоти

```bash
npm run assets:frames -- --locales en,uk,de
```

Без `--locales` збереться лише англійська. Щоб зібрати всі мови одразу, передай
повний список кодів із `src/i18n/index.ts`.

Результат — у `store-assets/play-frames/<мова>/`, по сім карток 1080 × 1920:
ключовий результат, форма, графік зростання, кругова діаграма, таблиця (темна),
список мов і методологія. Формат — 24-бітний PNG без альфа-каналу, саме як
вимагає Play.

Ці файли **не потрапляють у git** — 43 мови × 7 карток це близько 130 МБ, а
відтворюються вони однією командою. У репозиторії лежить рецепт, не результат.

### Що всередині карток

Вміст телефона намальовано під розмір магазинної мініатюри, а не зменшено зі
знімка екрана: у зменшеному знімку текст має висоту близько шести пікселів.
Але намальоване — не вигадане. Кожен підпис береться з файлів перекладу
застосунку, кожне число — з його ж модуля розрахунку на стандартних значеннях.
Play вимагає, щоб скриншоти показували справжній застосунок, тож нічого, чого
немає в компонентах, на картки не потрапляє.

---

## 6a. Назва бренду

Redempsly — торгова назва, а не зареєстрована особа. У юридичних текстах
застосунку названо ФОП Ihor Pokhyton; бренд лишається брендом.

Перед публікацією варто перевірити назву в реєстрах — це безкоштовно й займає
пів години. Інструкція: [TRADEMARK.md](TRADEMARK.md).
